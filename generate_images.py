...
from helpers import (
    ...
    create_gif,
    generate_audio_logo,  # Assuming these are defined in helpers
    generate_video_logo,
...
def generate_tab():
    ...
    if not replicate_api_key:
        st.warning("Please provide a valid Replicate API Key.")
        return
    ...
    with st.expander("Advanced Options"):
        ...
        bypass_images = st.checkbox("Bypass custom image prompts", value=True)
        
        if not bypass_images:
            auto_image = st.checkbox("Auto generate images", value=True)
        ...
        # Add Audio and Video Logo options
        add_audio_logo = st.checkbox("Add Audio Logo", value=True)
        add_video_logo = st.checkbox("Add Video Logo", value=True)
    ...
    if st.button("Generate Marketing Campaign"):
        with st.spinner("Generating..."):
            ...
            if not bypass_images:
                ...
                if auto_image:
                    custom_prompts = [generate_content("Generate image prompt", campaign_concept, budget, platforms, api_key)]
            ...
            # Generate audio logo if checked
            if add_audio_logo:
            ...
            # Generate video logo if checked
            if add_video_logo:
